      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 p-8 bg-gradient-to-br from-[var(--sidebar)] to-[var(--input-bg)] rounded-[32px] border border-[var(--border)] shadow-sm relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-6">
            <div>
              ...
              <h3 className="text-4xl font-black text-[var(--accent)] tracking-tighter">{user.character_class}</h3>
              <p className="text-sm font-bold text-gray-500 mt-1">Level {user.character_level} Technician</p>
            </div>
            
            <div className="space-y-3">
              <div className="flex justify-between text-[10px] font-black uppercase">
                <span className="text-gray-400">Progression</span>
                <span className="text-[var(--accent)]">{Math.floor(progress)}% to Level {user.character_level + 1}</span>
              </div>
              ...
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-bold text-gray-400 tracking-widest">{user.xp_total.toLocaleString()} TOTAL XP</span>
        ...
        <div className="lg:col-span-2 p-8 bg-[var(--sidebar)] border border-[var(--border)] rounded-[32px] shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            {[
              { label: 'Intelligence (INT)', val: stats.int, icon: Brain, color: 'text-blue-500' },
              { label: 'Strength (STR)', val: stats.str, icon: Swords, color: 'text-red-500' },
              { label: 'Wisdom (WIS)', val: stats.wis, icon: BookOpen, color: 'text-emerald-500' },
              { label: 'Dexterity (DEX)', val: stats.dex, icon: Zap, color: 'text-yellow-500' },
              { label: 'Constitution (CON)', val: stats.con, icon: Shield, color: 'text-cyan-500' },
              { label: 'Charisma (CHA)', val: stats.cha, icon: Heart, color: 'text-pink-500' },
            ].map(s => (
