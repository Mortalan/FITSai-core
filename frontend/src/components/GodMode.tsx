...
import { 
  ...
  Brain, DollarSign, FileText, Crown, Download, ChevronRight
} from 'lucide-react';
    ...
    <div className="flex h-full gap-8 animate-in fade-in duration-500">
      <div className="w-72 flex-shrink-0 space-y-6">
        ...
        <div className="bg-[var(--sidebar)] border border-[var(--border)] rounded-3xl p-3 shadow-sm space-y-1">
          {menuItems.map(item => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full flex items-center justify-between p-3.5 rounded-2xl transition-all ${activeTab === item.id ? 'bg-[var(--accent)] text-white shadow-lg' : 'text-gray-500 hover:bg-[var(--input-bg)]'}`}>
              <div className="flex items-center gap-3"><item.icon size={18} className={activeTab === item.id ? 'text-white' : item.color} /><span className="text-sm font-bold">{item.name}</span></div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 animate-in fade-in duration-300">
              ...
              <div className="xl:col-span-2">
                <div className="bg-black rounded-[32px] overflow-hidden border border-gray-800 shadow-2xl h-full min-h-[650px] flex flex-col">
                  ...
                  <div className="flex-1 p-8 overflow-y-auto custom-scrollbar font-mono text-[13px] leading-relaxed text-green-500/90"><pre className="whitespace-pre-wrap">{logs || `Momo Control v2.1.5\nSelect a service to stream journals...\n\nReady.`}</pre></div>
                </div>
              </div>
            </div>
          )}
