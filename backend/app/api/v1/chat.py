...
async def stream_momo(request: Request, background_tasks: BackgroundTasks, current_user: User = Depends(get_current_user)):
    ...
    async def event_generator():
        ...
        # Handle Tier 2 (Local LLM via Ollama)
        if tier == AITier.TIER2:
            try:
                ...
                ans = await local_llm_service.generate_response(sys_prompt, question)
                ...
                yield f"event: token\ndata: {json.dumps({'content': ans})}\n\n"
                yield f"event: done\ndata: {json.dumps({'status': 'finished', 'source': 'Local AI', 'xp_progress': progress, 'conversation_id': current_conv_id})}\n\n"
                return
            except Exception as e:
                logger.warning(f"Tier 2 fallback to Tier 3 due to local model failure: {e}")
                tier = AITier.TIER3 # Fallback

        # Handle Tier 3 (Agentic Engine with Tools)
        async with AsyncSessionLocal() as db:
        ...
        tool_count, full_answer, tokens_input, tokens_output = 0, "", 0, 0
        async for event in momo_engine.astream_events(initial_state, version="v2"):
            ...
            if kind == "on_chat_model_stream":
                ...
                async with AsyncSessionLocal() as db:
                    ...
                    yield f"event: done\ndata: {json.dumps({'status': 'finished', 'source': 'Cloud AI', 'xp_progress': progress, 'new_achievements': new_achievements, 'conversation_id': current_conv_id, 'nudge': nudge})}\n\n"
